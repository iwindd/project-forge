...
export function makeStore(preloadedState: PreloadedState) {
  const store = configureStore({
    ...
    preloadedState: {
      ...
      [api.reducerPath]: preloadedState.api ?? api.reducer(undefined, { type: '@@INIT' }),
    },
...
export async function createPreloadedState(auth: AuthState, organizations: Organization[]): Promise<PreloadedState> {
  const store = makeStore({ auth });

  await store.dispatch(organizationApi.util.upsertQueryData('getOrganizations', undefined, organizations));
