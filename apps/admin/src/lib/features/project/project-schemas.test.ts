...
describe('project resource schema', () => {
  ...
  it('rejects non database UUID identifiers', () => {
    ...
    expect(() =>
      projectSchema.parse({ ...project, organizationId: 'organization-id' })
    ).toThrow()
  })

  it('rejects an unknown status', () => {
    expect(() =>
      projectSchema.parse({ ...project, status: 'DELETED' })
    ).toThrow()
  })
...
describe('project response envelope schemas', () => {
  ...
  it('requires the project under the mutation envelope', () => {
    expect(
      projectResponseEnvelopeSchema.parse({ data: { project } })
    ).toEqual({ data: { project } })
    expect(() => projectResponseEnvelopeSchema.parse({ project })).toThrow()
    expect(() =>
      projectResponseEnvelopeSchema.parse({ data: { project: null } })
    ).toThrow()
  })
})
