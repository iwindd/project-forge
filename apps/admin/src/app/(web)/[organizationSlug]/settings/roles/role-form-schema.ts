...
export const organizationRoleFormSchema = z.object({
  name: z.string().trim().min(1, 'กรุณากรอกชื่อบทบาท').max(80, 'ชื่อบทบาทต้องไม่เกิน 80 ตัวอักษร'),
  permissions: z.array(z.enum(ORGANIZATION_PERMISSION_KEYS)).max(2, 'ไม่สามารถเลือกสิทธิ์ซ้ำได้'),
});

export type OrganizationRoleFormValues = z.infer<typeof organizationRoleFormSchema>;
