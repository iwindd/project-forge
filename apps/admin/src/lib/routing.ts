type RouteParamValue = string | number | readonly (string | number)[];
...
function createPathMatcher(path: string) {
  ...
  return new RegExp(`^${source}${normalizedPath === '/' ? '' : '/?'}$`, 'i');
}

function isParamList(value: RouteParamValue): value is readonly (string | number)[] {
  return Array.isArray(value);
}

function compilePath(path: string, params: RouteParams) {
  return normalizePath(path).replace(PARAMETER_PATTERN, (_, marker: string, name: string) => {
    ...
    if (marker === '*') {
      if (!isParamList(value) || value.length === 0) {
        throw new TypeError(`Wildcard route parameter "${name}" must be a non-empty array`);
      }
...
function routeSpecificity(path: string) {
  ...
  const dynamicSegments = segments.filter((segment) => /[:*][A-Za-z_$]/.test(segment)).length;
  const staticSegments = segments.length - dynamicSegments;

  return segments.length * 100 + staticSegments * 20 + path.length - parameters.length * 10 - wildcards.length * 20;
}

export function ROUTER(routeConfig: Record<string, RouteConfig>): Record<string, Route> {
  const routes: Record<string, Route> = {};

  function traverse(definitions: Record<string, RouteConfig>, parentKey?: string) {
...
export function buildRouteUtility(routes: Record<string, Route>) {
  const matchers = Object.values(routes)
    ...
    .sort((a, b) => {
      const specificity = routeSpecificity(b.route.path) - routeSpecificity(a.route.path);
  ...
  function getRoute(routeName: string) {
    const route = routes[routeName] ?? Object.values(routes).find((candidate) => candidate.name === routeName);
