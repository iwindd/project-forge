...
export function SecurityLogsTable({ userId }: { userId?: string }) {
  ...
  const queryArg = userId && organizationId ? { userId, organizationId } : {};
  const { data, isLoading, isError, refetch } = useGetSecurityLogsQuery(queryArg, { skip: missingOrganizationScope });
            ...
            {logs.length ? (
              logs.map((log) => (
                <Table.Tr key={log.id}>
                  ...
                  <Table.Td>{formatCreatedAt(log.createdAt)}</Table.Td>
                </Table.Tr>
              ))
            ) : (
              <Table.Tr>
