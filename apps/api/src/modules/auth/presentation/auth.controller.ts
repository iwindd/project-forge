...
export class AuthController {
  ...
  @Get('github/callback')
  async callback(@Query() query: unknown, @Req() request: Request, @Res() response: Response) {
    ...
    if (!parsedQuery.success || parsedQuery.data.state !== expected) {
      ...
      return response.redirect(this.appRedirect('/login?error=invalid_oauth_state'));
    }
    const returnToResult = githubReturnToSchema.safeParse(getCookie(request, 'pf_oauth_return_to'));
    ...
    try {
      ...
      const destination =
        returnTo ?? (result.organizationSlug ? `/${encodeURIComponent(result.organizationSlug)}` : '/account');
      ...
      return response.redirect(this.appRedirect(`/login?error=${encodeURIComponent(message.slice(0, 120))}`));
    }
  }
  ...
  async updateMe(@Principal() principal: AuthenticatedPrincipal, @Body() body: unknown) {
    ...
    return authUpdateMeResponseSchema.parse(
      apiSuccess({
        user: {
          ...data.user,
  ...
  async logoutSession(@Req() request: Request, @Res() response: Response) {
    ...
    response.clearCookie('pf_session', { path: '/' });
    return response.status(200).json(apiNullSuccessResponseSchema.parse(apiSuccess(null)));
  }
}
