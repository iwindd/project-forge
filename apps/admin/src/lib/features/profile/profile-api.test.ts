...
describe('profile browser API contracts', () => {
  ...
  it('transforms the canonical profile and derives identity from GitHub connections', async () => {
    ...
    const result = await createStore().dispatch(profileApi.endpoints.getProfile.initiate());
  ...
  it('encodes a connection id and validates the null disconnect response', async () => {
    ...
    const result = await createStore().dispatch(profileApi.endpoints.disconnectConnection.initiate('connection/id'));
  ...
  it('rejects malformed mutation output instead of showing success', async () => {
    ...
    const result = await createStore().dispatch(profileApi.endpoints.updateProfile.initiate({ bio: null }));
