...
import type { PermissionKey, PermissionMode } from '@/lib/permissions';
...
function filterNavItems(items: NavItem[], can: PermissionCheck): NavItem[] {
  return items.flatMap((item) => {
    if (item.permissionKey && !can(item.permissionKey, item.permissionMode)) {
      return [];
    }
...
function filterNavGroups(groups: readonly NavGroup[], can: PermissionCheck): NavGroup[] {
...
function useFilteredNavGroups(): NavGroup[] {
  ...
  return filterNavGroups(organizationNavigation, can);
}

export function useNavigationGroups(mode: SidebarNavigationMode = 'app'): NavGroup[] {
...
export function getLinkHref(item: NavItem, params: RouteParams = {}): NavLinkHref {
  if (!item.href) {
...
export function useIsRouteActive() {
  ...
  return (routeName?: string) => Boolean(routeName && activeRouteTrail.some((route) => route.name === routeName));
}
...
export function checkActiveChildren(children: NavItem[], isActiveRoute: (routeName?: string) => boolean): boolean {
  return children.some(
    (child) =>
      (isLinkItem(child) && isActiveRoute(child.routeName)) ||
      (isParentItem(child) && checkActiveChildren(child.items, isActiveRoute)),
  );
}
