...
The pull-request workflow runs the repository check plus the API and Admin gates, installs
Chromium, and runs the browser acceptance suite. Any pre-existing lint or formatting warning must
remain visible in the command output and must be reported separately from failures introduced by
the change.
...
- `apps/**/test-results/**` is generated by Playwright and is excluded from source checks.
