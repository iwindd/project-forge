...
describe('github repository URL validation', () => {
  it('accepts normalized GitHub HTTPS repository URLs', () => {
    expect(isGithubHttpsRepositoryUrl('https://github.com/owner/repository')).toBe(true);
    expect(isGithubHttpsRepositoryUrl('https://github.com/owner/repository.git')).toBe(true);
    expect(isGithubHttpsRepositoryUrl('https://GitHub.com/owner/repository')).toBe(true);
  });

  it('rejects URLs the API domain rejects', () => {
    expect(isGithubHttpsRepositoryUrl('http://github.com/owner/repository')).toBe(false);
    expect(isGithubHttpsRepositoryUrl('https://user@github.com/owner/repository')).toBe(false);
    expect(isGithubHttpsRepositoryUrl('https://github.com/owner/repository?ref=main')).toBe(false);
    expect(isGithubHttpsRepositoryUrl('https://github.com/owner/repository#readme')).toBe(false);
    expect(isGithubHttpsRepositoryUrl('https://gitlab.com/owner/repository')).toBe(false);
    expect(isGithubHttpsRepositoryUrl('https://github.com/owner')).toBe(false);
    expect(isGithubHttpsRepositoryUrl('https://github.com/owner/repo/sub')).toBe(false);
    expect(isGithubHttpsRepositoryUrl('https://github.com/ow ner/repository')).toBe(false);
...
describe('project form schema', () => {
  ...
  it('mirrors the API bounds and required fields', () => {
    expect(() => formSchema.parse({ ...validValues, name: 'a'.repeat(121) })).toThrow();
    expect(() => formSchema.parse({ ...validValues, githubUrl: '' })).toThrow();
    expect(() => formSchema.parse({ ...validValues, sourceBranch: 'a'.repeat(121) })).toThrow();
    expect(() => formSchema.parse({ ...validValues, nodeVersion: 'a'.repeat(41) })).toThrow();
  });
...
describe('project form submission', () => {
  it('sends environment variable names with the masked value', () => {
    expect(parseEnvironmentMetadata('DATABASE_URL\nAPI_KEY\n\nDATABASE_URL')).toEqual({
  ...
  it('drops entries whose name is not a valid environment variable name', () => {
    expect(parseEnvironmentMetadata('API KEY\nAPI-KEY\nAPI$KEY\n1API_KEY\nAPI.KEY\n=value')).toEqual({});
  });

  it('keeps only the valid lines when valid and invalid lines are mixed', () => {
    expect(parseEnvironmentMetadata('DATABASE_URL=postgres://secret\nAPI-KEY=secret')).toEqual({
      DATABASE_URL: 'configured',
    });
  });
  ...
  ])('drops the credential-shaped bare token %s instead of storing it as a key name', (token) => {
