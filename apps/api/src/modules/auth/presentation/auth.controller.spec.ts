...
describe('AuthController', () => {
  it('stores a validated OAuth continuation path', () => {
    ...
    controller.start({ returnTo: '/invitations/invite-token' }, response as never);
  ...
  it('redirects an approved OAuth login to the resolved organization route', async () => {
    ...
    await controller.callback(
      ...
      {
        headers: {
          cookie: 'pf_oauth_state=expected-state; pf_oauth_return_to=%2Finvitations%2Finvite-token',
        },
      } as never,
      response as never,
    );

    expect(response.redirect).toHaveBeenCalledWith('http://localhost:5051/invitations/invite-token');
    expect(response.cookie).toHaveBeenCalledWith(
      'pf_session',
      'session-token',
      expect.objectContaining({ httpOnly: true }),
  ...
  it('records invalid OAuth state without recording OAuth values', async () => {
    ...
    expect(response.redirect).toHaveBeenCalledWith('http://localhost:5051/login?error=invalid_oauth_state');
  ...
  it('keeps a session cookie for an OAuth login without organization access', async () => {
    ...
    expect(response.redirect).toHaveBeenCalledWith('http://localhost:5051/account');
  });
