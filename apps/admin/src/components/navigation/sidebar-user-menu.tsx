    ...
    <Menu position='top-start' offset={8} width={260} shadow='md' withinPortal>
      <Menu.Target>
        <UnstyledButton className={classes.control} aria-label={displayName}>
          ...
          <IconChevronUp className={classes.chevron} size={17} stroke={1.8} aria-hidden='true' />
        </UnstyledButton>
      </Menu.Target>

      <Menu.Dropdown>
        <Menu.Label>{user.email}</Menu.Label>
        <Menu.Item component={Link} href={getPath('account.settings')} leftSection={<IconUserCircle size={17} />}>
          {t('myAccount')}
        </Menu.Item>
        <Menu.Divider />
        <Menu.Item
