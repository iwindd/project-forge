...
import { apiNullSuccessResponseSchema, apiSuccess } from '../../../common/http/api-response.js';
...
export class OrganizationsController {
  ...
  @Get(':id/roles')
  async roles(@Principal() principal: AuthenticatedPrincipal, @Param() rawParams: unknown) {
  ...
  async createRole(
    ...
    const role = await this.createOrganizationRole.execute(principal.id, organizationId, input, {
  ...
  async updateRole(
    ...
    const role = await this.updateOrganizationRole.execute(principal.id, organizationId, roleId, input, {
      requestId: getRequestId(request),
    });
    return apiSuccess(organizationRoleResponseSchema.parse({ role }));
  }
  ...
  async deleteRole(
    ...
    const { id: organizationId, roleId } = organizationRoleParamSchema.parse(rawParams);
    const result = await this.deleteOrganizationRole.execute(principal.id, organizationId, roleId, {
      requestId: getRequestId(request),
    });
  ...
  async members(
    ...
    const result = await this.listOrganizationMembers.execute(principal.id, organizationId, query);
  ...
  async update(
    ...
    const organization = await this.organizations.updateOrganization(principal.id, organizationId, input.name, {
      requestId: getRequestId(request),
    });
    return apiSuccess(
      organizationResponseSchema.parse({
  ...
  async updateMember(
    ...
    const membership = await this.organizations.updateMemberRole(principal.id, organizationId, userId, input, {
      requestId: getRequestId(request),
    });
  ...
  async member(@Principal() principal: AuthenticatedPrincipal, @Param() rawParams: unknown) {
    const { id: organizationId, userId } = organizationMemberParamSchema.parse(rawParams);
    const user = await this.organizations.getMember(principal.id, organizationId, userId);
  ...
  async memberStatus(
    ...
    const user = await this.organizations.updateMemberStatus(principal.id, organizationId, userId, input.active, {
      requestId: getRequestId(request),
    });
    return apiSuccess(organizationMemberUserResponseSchema.parse({ user }));
  }
  ...
  @Get(':id/invitations')
  async invitations(@Principal() principal: AuthenticatedPrincipal, @Param() rawParams: unknown) {
    const { id: organizationId } = organizationIdParamSchema.parse(rawParams);
    return apiSuccess(
      organizationInvitationListSchema.parse(await this.organizations.listInvitations(principal.id, organizationId)),
    );
  }
  ...
  async cancelInvitation(
    ...
    await this.cancelOrganizationInvitation.execute(principal.id, organizationId, invitationId, {
      requestId: getRequestId(request),
    });
