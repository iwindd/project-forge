...
describe('DeleteOrganizationRoleUseCase', () => {
  it('delegates role deletion to the organization application service', async () => {
    ...
    await expect(useCase.execute('actor-id', 'organization-id', 'role-id', { requestId: 'request-id' })).resolves.toEqual({ ok: true });
    expect(organizations.deleteRole).toHaveBeenCalledWith('actor-id', 'organization-id', 'role-id', { requestId: 'request-id' });
  });
});
