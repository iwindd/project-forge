...
describe('AuditLogsController', () => {
  it('validates audit date filters as calendar dates and ordered ranges', () => {
    ...
    expect(() =>
      auditLogQuerySchema.parse({ from: '2026-02-30' })
    ).toThrow()
    expect(() =>
      auditLogQuerySchema.parse({ from: '2026-09-10', to: '2026-09-01' })
    ).toThrow()
  })
  ...
  it('returns security logs in the standard success envelope', async () => {
    ...
    expect(response).toEqual({
      data: [
          expect.objectContaining({ id: auditLogId, event: 'LOGIN_SUCCEEDED' })
      ],
