...
export type SecurityLogsArg = { userId?: never; organizationId?: never } | { userId: string; organizationId: string };

export function getSecurityLogsTag(arg: SecurityLogsArg) {
  return {
    type: 'SecurityLogs' as const,
    id: 'userId' in arg && arg.userId ? `${arg.organizationId}:${arg.userId}` : 'own',
  };
}
