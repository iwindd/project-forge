...
describe('auth session contract', () => {
  beforeEach(() => {
    ...
    mocks.apiServerFetch.mockImplementation(async (_path: string, schema: { parse: (value: unknown) => unknown }) =>
  ...
  it('maps an unauthenticated API response to an empty session', async () => {
    const error = new ApiServerError(401, 'UNAUTHENTICATED', 'Please sign in with GitHub', {}, 'request-401');
  ...
  it('propagates server failures instead of turning them into login redirects', async () => {
    const error = new ApiServerError(500, 'INTERNAL_SERVER_ERROR', 'Internal server error', {}, 'request-500');
    mocks.apiServerFetch.mockRejectedValue(error);
