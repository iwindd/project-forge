...
import { OrganizationMemberRole } from '../../organizations/domain/organization.js';
...
export class AuditLogsController {
  ...
  @Get('security/me')
  listMySecurityLogs(@Principal() principal: AuthenticatedPrincipal, @Query() query: AuditQuery) {
  ...
  async exportMine(
    ...
    const log = await this.em.findOne(AuditLogOrmEntity, { id });
    if (!log || (log.actorId !== principal.id && log.targetUserId !== principal.id)) {
  ...
  private async queryLogs(
    query: AuditQuery,
    scope: 'all' | { userId: string } | { organizationId: string } | { organizationId: string; userId: string },
  ) {
    ...
    if (scope !== 'all' && 'userId' in scope) andConditions.push({ $or: [{ actorId: scope.userId }, { targetUserId: scope.userId }] });
  ...
  private async requireOrganizationViewer(principal: AuthenticatedPrincipal, organizationId: string) {
    ...
    const { membership } = await this.organizations.requireMembership(principal.id, organizationId);
    if (![OrganizationMemberRole.OWNER, OrganizationMemberRole.ADMIN].includes(membership.role)) {
      throw new ForbiddenException({ code: 'ORGANIZATION_ADMIN_REQUIRED', message: 'Organization administrator access is required' });
    }
    return membership;
  }
}
