...
describe('organization route and preload seam', () => {
  ...
  it('preloads the organizations and sends the route-resolved id to the browser API', async () => {
    const selected = resolveOrganizationFromRoute(organizations, 'organization-b', organizations[1].id);
    expect(selected?.id).toBe(organizations[1].id);

    const preloadedState = await createPreloadedState({ user: { id: 'user-id', role: 'EDITOR' } }, organizations);
    expect(preloadedState.api?.queries['getOrganizations(undefined)']).toMatchObject({
      data: organizations,
      status: 'fulfilled',
    });
    ...
    expect(requestUrl).toContain(`/organizations/${organizations[1].id}/members`);
  });
});
