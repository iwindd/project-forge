...
describe('ListConnectedPullRequestsUseCase', () => {
  it('resolves only the authenticated user GitHub connection and never returns its token', async () => {
    ...
    const em = { findOne: vi.fn().mockResolvedValue({ userId: 'user-1', provider: 'GITHUB', providerUsername: 'forge', accessTokenCiphertext: 'cipher' }) };
    const useCase = new ListConnectedPullRequestsUseCase(em as never, github as never, { decrypt: vi.fn().mockReturnValue('secret'), encrypt: vi.fn() } as never);
    await expect(useCase.execute('user-1', 1, 20)).resolves.toEqual({ items: [], page: 1, perPage: 20, hasNextPage: false });
  ...
  it('maps a missing connection to not found', async () => {
    const useCase = new ListConnectedPullRequestsUseCase({ findOne: vi.fn().mockResolvedValue(null) } as never, {} as never, {} as never);
