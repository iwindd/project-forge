...
function TabNavigation({ ariaLabel, items, onNavigate }: SidebarTabsProps & { onNavigate?: () => void }) {
  const pathname = usePathname();

  return (
    <Tabs value={pathname} variant='none' orientation='vertical' placement='left' w='100%' classNames={classes}>
...
export function SidebarTabs({ ariaLabel, drawerTitle, items }: SidebarTabsProps) {
  ...
  return (
    <>
      ...
      >
        <TabNavigation ariaLabel={ariaLabel} drawerTitle={drawerTitle} items={items} onNavigate={close} />
      </Drawer>
    </>
  );
}
