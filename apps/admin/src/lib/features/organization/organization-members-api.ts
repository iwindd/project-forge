...
export type OrganizationRoleSummary = z.infer<typeof organizationRoleSummarySchema>;

export type OrganizationMember = z.infer<typeof organizationMemberSchema>;

export type OrganizationMemberUser = z.infer<typeof organizationMemberUserResponseSchema>['user'];
...
type CreateInvitationResponse = z.infer<typeof organizationInvitationResponseSchema>;
...
export function parseRoleResponse(response: unknown, meta: ApiMetaCarrier | undefined): OrganizationRolesResponse {
...
export function parseMembersResponse(response: unknown, meta: ApiMetaCarrier | undefined): OrganizationMembersResponse {
...
export function parseAcceptInvitationResponse(response: unknown): AcceptInvitationResponse {
...
export const organizationMembersApi = api.injectEndpoints({
  endpoints: (builder) => ({
    getRoles: builder.query<OrganizationRolesResponse, { organizationId: string }>({
      query: ({ organizationId }) => `organizations/${encodeURIComponent(organizationId)}/roles`,
      transformResponse: parseRoleResponse,
      providesTags: (_result, _error, { organizationId }) => [{ type: 'OrganizationRoles', id: organizationId }],
    }),
    createRole: builder.mutation<{ role: z.infer<typeof organizationRoleSchema> }, CreateRoleInput>({
      ...
      invalidatesTags: (_result, _error, { organizationId }) => [{ type: 'OrganizationRoles', id: organizationId }],
    }),
    updateRole: builder.mutation<{ role: z.infer<typeof organizationRoleSchema> }, UpdateRoleInput>({
    ...
    getMembers: builder.query<OrganizationMembersResponse, { organizationId: string; query: OrganizationMembersQuery }>(
      {
        ...
        providesTags: (_result, _error, { organizationId }) => [{ type: 'OrganizationMembers', id: organizationId }],
      },
    ),
    getInvitations: builder.query<OrganizationInvitation[], { organizationId: string }>({
      query: ({ organizationId }) => `organizations/${encodeURIComponent(organizationId)}/invitations`,
      transformResponse: (response: unknown) => z.array(organizationInvitationSchema).parse(response),
      providesTags: (_result, _error, { organizationId }) => [{ type: 'OrganizationInvitations', id: organizationId }],
    }),
    ...
    updateMemberRole: builder.mutation<{ membership: OrganizationMemberUser }, UpdateMemberRoleInput>({
    ...
    updateMemberStatus: builder.mutation<{ user: OrganizationMemberUser }, UpdateMemberStatusInput>({
      ...
      invalidatesTags: (_result, _error, { organizationId }) => [{ type: 'OrganizationMembers', id: organizationId }],
    }),
    ...
    acceptInvitation: builder.mutation<AcceptInvitationResponse, AcceptInvitationInput>({
      query: ({ token }) => ({
