...
เว็บอยู่ที่ `http://localhost:3006` และ API อยู่ที่ `http://localhost:3007/api/v1` ส่วน PostgreSQL ของ Project Forge ใช้ host port `55432` เพื่อไม่ชนกับ project-bot ที่ใช้ `5433`

ตั้งค่า GitHub OAuth ใน `.env` โดยใช้ callback:

```text
http://localhost:3007/api/v1/auth/github/callback
```
